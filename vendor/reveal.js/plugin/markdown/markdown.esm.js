    ...
    return (n, s, r) => {
        ...
        if (s.async)
            return Promise.resolve(s.hooks ? s.hooks.preprocess(n) : n)
                ...
                .then((e) =>
                    s.walkTokens
                        ? Promise.all(M.walkTokens(e, s.walkTokens)).then(
                              () => e
                          )
                        : e
                )
...
;(M.options = M.setOptions =
    function (e) {
        var n
        return (
            (M.defaults = { ...M.defaults, ...e }), (n = M.defaults), (t = n), M
        )
    }),
    (M.getDefaults = e),
    (M.defaults = t),
            ...
            switch (((n = n.concat(t.call(M, s))), s.type)) {
                case 'table':
                    for (const e of s.header)
                        n = n.concat(M.walkTokens(e.tokens, t))
                    for (const e of s.rows)
                        for (const s of e)
                            n = n.concat(M.walkTokens(s.tokens, t))
                    break
                case 'list':
                ...
                default:
                    ...
                    M.defaults.extensions.childTokens[s.type]
                        ? M.defaults.extensions.childTokens[s.type].forEach(
                              function (e) {
